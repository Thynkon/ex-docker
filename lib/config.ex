defmodule ExDocker.Config do
  ...
  @doc """
  Given a %ExDocker.Config{} struct, formats and returns a dictionary of the appropiate options
  for creating a container.
  """
  def create_container(conf = %ExDocker.Config{generic_hostname: nil}) do
    %ExDocker.Config{conf | generic_hostname: false} |> create_container
  end

  def create_container(conf = %ExDocker.Config{generic_hostname: false}) do
  ...
  def create_container(conf = %ExDocker.Config{}) do
    %{
  ...
  @doc """
  Given a %ExDocker.Config{} struct, formats and returns a dictionary of the appropriate
  options for starting a container.
  """
  def start_container(conf = %ExDocker.Config{ports: %{}}) do
    port_map =
      conf.ports
      |> Map.values()
  ...
  def start_container(conf = %ExDocker.Config{}) do
    %{"Binds" => format_volumes(conf.volumes)}
  end
